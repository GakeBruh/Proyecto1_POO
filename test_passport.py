...
class TestPersonPassport(unittest.TestCase):
    ...
    def test_relacion(self):
        ...
        self.test_ids.append(ObjectId(persona_id))  # guardamos para limpieza

        pasaporte = Passport(
            number="T123456",
            country="Honduras",
            expiration_date=date(2030, 1, 1),
            person_id=persona_id  # usamos el _id de Mongo
        )
    ...
    def test_actualizacion(self):
        ...
        update_result = self.persons_coll.update_one(
            {"_id": ObjectId(persona_id)},
            {"$set": {"passport_id": ObjectId(passport_id)}}
        )
        self.assertEqual(update_result.modified_count, 1)

        updated = self.persons_coll.find_one({"_id": ObjectId(persona_id)})
        self.assertEqual(updated["passport_id"], ObjectId(passport_id))
